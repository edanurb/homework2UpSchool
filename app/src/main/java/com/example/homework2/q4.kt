...
fun main() {
    /**
     * bu soruda bazı kısımları değiştirdim sebepleri:
     *  hashing "val hashingAlgorithm: String" nullable olmadığı için hata veriyordu
     */
   ...
   coinList.map { CoinUI(it.name,it.coinId,it.description,it.image) }.forEach{
       println("name: ${it.name},  coinId: ${ it.coinId }, description: ${it.description}, image:${it.image}")
   }
}